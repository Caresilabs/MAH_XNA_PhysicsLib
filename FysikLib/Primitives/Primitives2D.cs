...
namespace C3.XNA
{
	...
	public static class Primitives2D
	{
        ...
        private static List<Vector2> CreateHalfCircle(double radius, int sides)
        {
            // Look for a cached version of this circle
            String circleKey = radius + "x" + sides;
            if (circleCache.ContainsKey(circleKey))
            ...
            for (double theta = -Math.PI/2; theta < max; theta += step)
            {
                vectors.Add(new Vector2((float)(radius * Math.Cos(theta)), (float)(radius * Math.Sin(theta))));
            }
            // Cache this circle so that it can be quickly drawn next time
            vectors.Add(new Vector2((float)(radius * Math.Cos(Math.PI / 2)), (float)(radius * Math.Sin(Math.PI / 2))));
            circleCache.Add(circleKey, vectors);
        ...
        public static void DrawHalfCircle(this SpriteBatch spriteBatch, Vector2 center, float radius, int sides, Color color, float thickness)
        {
            DrawPoints(spriteBatch, center, CreateHalfCircle(radius, sides), color, thickness);
        }
